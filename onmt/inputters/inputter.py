...
from onmt.inputters.sbn_dataset import SBNDataset
...
def get_fields(data_type, n_src_features, n_tgt_features):
    ...
    if data_type == 'text':
        ...
        return SBNDataset.get_fields(n_src_features, n_tgt_features)
    else:
...
def get_num_features(data_type, corpus_file, side):
    ...
    if data_type == 'text':
        ...
        return SBNDataset.get_num_features(corpus_file, side)
    else:
        raise ValueError("Data type not implemented")
...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
    ...
    def _make_examples_nfeats_tpl(data_type, src_data_iter, src_path, src_dir,
        ...
        if data_type == 'text':
            ...
            src_examples_iter, num_src_feats = \
                SBNDataset.make_SBN_examples_nfeats_tpl(
    ...
    if data_type == 'text':
        ...
        dataset = SBNDataset(fields, src_examples_iter, tgt_examples_iter,
                              num_src_feats, num_tgt_feats,
                              src_seq_length=src_seq_length,
                              tgt_seq_length=tgt_seq_length,
                              dynamic_dict=dynamic_dict,
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    if data_type == 'text' or data_type == 'SBN':
...
class SBNIterator(torchtext.legacy.data.Iterator):
    def __init__(self, dataset, batch_size, sort_key=None, device=None,
        ...
        super(SBNIterator, self).__init__(dataset=dataset, batch_size=batch_size, sort_key=sort_key,
    ...
    def __iter__(self):
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                batch = SBNBatch(minibatch, self.dataset, self.device, self.train)
                yield batch
...
def lazily_load_dataset(corpus_type, opt):
    ...
    def _lazy_dataset_loader(pt_file, corpus_type):
        logger.info('Loading %s dataset from %s, number of examples' % (corpus_type, pt_file))
    ...
    if pts:
        ...
        print("This is the data file {0} will be trained by Model".format(pt))
        yield _lazy_dataset_loader(pt, corpus_type)
